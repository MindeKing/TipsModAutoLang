...
set "config=AutoLang_Config.txt"
set "defaults=AutoLang_Defaults.txt"
...
if "!ui_color!"=="" (
	echo.
	echo If you don't want to be asked this every time,
	echo open the .bat file with any text editor and put "Light" or "Dark" after the = in the
	echo set "Light_or_Dark_mode="
	echo line at the top of the script.
		...
		if not "!line!"=="" (
			...
			if errorlevel 1 (
				...
				for /f "tokens=1,* delims= " %%A in ("!line!") do (
					...
					set /a defaults_counter=defaults_counter+1
				)
			)
		)
	)
...
if exist "!config!" (
		...
		if not "!line!"=="" (
			...
			if errorlevel 1 (
				...
				for /f "tokens=1,* delims= " %%A in ("!line!") do (
					...
					set "%%A_set=2"
					if defined %%A (set /a overwritten_counter=overwritten_counter+1)
					...
					set /a config_counter=config_counter+1
				)
			)
		)
	)
	...
	) else if !config_counter! GEQ 2 (!config_counter! settings loaded from "!config_name!".)
...
REM ==================== Terms ====================
REM __tip_folder_name [any valid string]
set "TFol=!__tip_folder_name!"
REM __lang_folder_name [any valid string]
...
echo CTKey is "!__custom_title_key!" & REM Still needs logic that keeps it from generating. (Or I could just set its default to the Tips mod default, "tipsmod.title.default", so it's easier to configure.)
echo ==================== General ====================
...
REM Read every file in the TipsFolderPath and output a .json file based on those files.
REM This is very obviously a problem if you want the user to be able to use different file extensions.
title !title_default_og! is writing to "!__lang_file_name!_temp!__lang_file_ext!"...
echo {> "!LFiP_temp!"
if defined CTKey (
	echo     "!CTKey!": {"translate": "placeholder",
	echo         "underlined": true,
	echo         "bold": true,
	echo         "color": "yellow"
	echo     },
)>> "!LFiP_temp!"
REM Collect filenames into a list.
...
REM Needs to read the contents of every file in the tips folder to find the...
REM "text": {"translate": "tip.cust_title_test"
REM ...string, and then use the third piece of quoted text as the key variable.
echo.
REM Loop through the list and output valid JSON
:LFiP_temp_loop
for /f "tokens=1* delims=*" %%A in ("!file_list!") do (
	echo Searching in "%%~A" for text translate reference key.
				...
				if not errorlevel 1 (
					for /f "tokens=2 delims=:" %%T in ("!line!") do (
						...
						if defined file_list (
							echo     "!key!": "placeholder",>> "!LFiP_temp!"
							goto :LFiP_temp_loop
						) else (
							echo     "!key!": "placeholder">> "!LFiP_temp!"
						)
...
echo.
echo Finished searching all "!TFiExt!" files in the "!__tip_folder_name!" folder.
REM Output final closed bracket.
echo }>> "!LFiP_temp!"
if exist "!LFiP!" (del "!LFiP!")
...
REM __tip_folder_name [any valid string]
	...
	call :processing_md "__tip_folder_name" 0 "" "pmd_msg_set" "pmd_msg_success" "pmd_msg_blank" 0
...
REM __lang_folder_name [any valid string]
	...
	call :processing_md "__lang_folder_name" 0 "" "pmd_msg_set" "pmd_msg_success" "pmd_msg_blank" 0
...
REM __lang_file_template [path to existing .txt / blank]
	set "pmd_msg_set=__lang_file_template: "
	set "pmd_msg_blank=__lang_file_template blank"
	call :processing_md "__lang_file_template" 1 ".txt" "pmd_msg_set" "pmd_msg_success" "pmd_msg_blank" 1
